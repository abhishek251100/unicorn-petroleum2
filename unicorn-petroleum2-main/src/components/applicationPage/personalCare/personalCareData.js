export const personalCareData = {
  hero: { title: 'Personal Care Applications', description: 'Ingredients for daily care products' },
  slider: [
    { title: 'Personal Care Applications', subtitle: 'Ingredients for daily care products', image: '/assets/Application%20Banner/Personal%20care%20application.jpg', mobileImage: '/assets/Application%20Banner/Personal%20care%20application.jpg' },
  ],
  overview: {
    title: 'Personal Care',
    description:
      "Our products have a substantial influence on generic personal care formulas. They help to retain moisture, increase suppleness, and facilitate delicate treatment to one's skin and hair. These materials contribute to product durability and increased shelf life in both day-to-day usage and wash-off formulations. These are extensively used in dermatological applications, hair treatment, pediatric treatment, dental care, and other personal hygiene products. They also find their use in helping with cleaning, conditioning, and protection in daily activities. Their steady functioning assures continuous quality in consumer goods allowing manufacturers to furnish dependable and efficient personal care solutions.",
  },
  relatedProducts: [
    { name: 'Petroleum Jelly', image: '/assets/RELATED PROD IMG/NEW IMAGES/Petroleumjelly.jpg', hoverImage: '/assets/RELATED PROD IMG/New Hover Images/Petroleumjelly.jpg', description: 'Healing ointments, skin protectants, and excipients in topical medicines.' },
    { name: 'White Mineral Oils', image: '/assets/RELATED PROD IMG/NEW IMAGES/whiteMineraloil1.jpg', hoverImage: '/assets/RELATED PROD IMG/New Hover Images/WhiteMineraloil.jpg', description: 'Used as a laxative, tablet coating, ointment base, and in topical formulations.' },
    { name: 'Natural Beeswax', image: '/assets/RELATED PROD IMG/NEW IMAGES/Naturalbeeswax.jpg', hoverImage: '/assets/RELATED PROD IMG/New Hover Images/beeswax2.jpg', description: 'Used in lotions, creams and lip balms for its emollient and protective benefits' },
    { name: 'Emulsifying Wax', image: '/assets/RELATED PROD IMG/NEW IMAGES/emulsifyingwax.jpg', hoverImage: '/assets/RELATED PROD IMG/New Hover Images/emulsifying2.jpg', description: 'Used in emulsified medicinal creams.' },
    { name: 'Hard Paraffin Wax', image: '/assets/RELATED PROD IMG/NEW IMAGES/paraffinwax.jpg', hoverImage: '/assets/RELATED PROD IMG/New Hover Images/paraffin2.jpg', description: 'Used in creams, ointments, balms, and lip care products.' },
    { name: 'D-Panthenol', image: '/assets/RELATED PROD IMG/NEW IMAGES/Dpanthenol.jpg', hoverImage: '/assets/RELATED PROD IMG/New Hover Images/Panthenol.jpg', description: 'Provitamin of B5 supporting skin regeneration and hydration.' },
    { name: 'Lanolin', image: '/assets/RELATED PROD IMG/NEW IMAGES/lanolin.jpg', hoverImage: '/assets/RELATED PROD IMG/NEW IMAGES/lanolin.jpg', description: 'Refined wool grease used as a skin-conditioning excipient in cosmetic and topical formulations.' },
    { name: 'UV Filters', image: '/assets/RELATED PROD IMG/NEW IMAGES/uvfilters.jpg', hoverImage: '/assets/RELATED PROD IMG/New Hover Images/uvFilters.jpg', description: 'Used in dermatological formulations to protect against UV‑induced damage.' },
    { name: 'Preservatives', image: '/assets/RELATED PROD IMG/NEW IMAGES/Preservative.jpg', hoverImage: '/assets/RELATED PROD IMG/New Hover Images/preservative.jpg', description: 'Prevent microbial growth in syrups, eye drops, ointments, and injectables.' },
    { name: 'Surfactants', image: '/assets/RELATED PROD IMG/NEW IMAGES/surfactant.jpg', hoverImage: '/assets/RELATED PROD IMG/New Hover Images/surfactant.jpg', description: 'Additives in formulations for dispersing and cleaning functions.' },
  ],
};

export default personalCareData;


