        ...
        {centerLastRow && cardsCount % 4 === 3 ? (
          ...
          <div className={gridClasses}>
            {data.cards.map((card, index) => (
              <div
                key={index}
                className="bg-white p-6 rounded-2xl border border-[#EDA94E] hover:shadow-xl hover:border-[#EDA94E] transition-all duration-300 transform hover:-translate-y-1 cursor-pointer group text-center flex flex-col"
              >
                {/* Icon */}
                <div className="flex-1 flex items-center justify-center text-[#EDA94E] text-4xl mb-3 group-hover:scale-110 transition-transform duration-300">
                  {card.icon && card.icon.startsWith('/') ? (
